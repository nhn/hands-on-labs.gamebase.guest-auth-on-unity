...
extern "C" {
    ...
    char* getSync(char* jsonString) {
        [TCGBUtil logDebugWithFormat:@"[TCGB][Plugin][UnityMessageReceiver] getSync jsonString : %s", jsonString];
        NSString *data;
        if(jsonString != nil)
        ...
        message.requestMethodName = convertedDic[@"requestMethodName"];
    ...
    void getAsync(char* jsonString) {
        [TCGBUtil logDebugWithFormat:@"[TCGB][Plugin][UnityMessageReceiver] getAsync jsonString : %s", jsonString];
        NSString *data;
        if(jsonString != nil)
            data = [NSString stringWithUTF8String:jsonString];
