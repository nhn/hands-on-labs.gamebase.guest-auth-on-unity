...
-(void)queryPush:(UnityMessage*)message {
    ...
    [TCGBPush queryPushWithCompletion:^(TCGBPushConfiguration *configuration, TCGBError *error) {
        NativeMessage* requestMessage = [[NativeMessage alloc]initWithMessage:message.scheme handle:message.handle TCGBError:error jsonData:[configuration JSONString] extraData:nil];
        [[UnityMessageSender sharedUnityMessageSender] sendMessage:requestMessage gameObjectName:message.gameObjectName requestMethodName:message.requestMethodName];
    }];
}
