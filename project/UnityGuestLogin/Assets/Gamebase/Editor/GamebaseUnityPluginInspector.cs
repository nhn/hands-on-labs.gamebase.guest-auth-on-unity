...
#region App Settings
    ...
    private GUIContent  useWebviewContent           = null;
#endregion

#region Platform
    private bool        platformFoldout             = true;
    private bool        iosFoldout                  = true;
    private bool        aosFoldout                  = true;
...
#region Popup
    ...
    private void CreateContent()
    {
        ...
        if (null == useWebviewContent)
        {
            useWebviewContent           = new GUIContent("Use Webview", "Settings for using webview on Gamebase.");
        }        
        ...
        if (null == fcmContent)
        {
            fcmContent                  = new GUIContent("FCM Sender ID", "Sender ID for Firebase Cloud Messaging (FCM) use.");
        }
    ...
    private void DrawPlatformSettings()
    {
        ...
        if (true == platformFoldout)
        {
            ...
            aosFoldout = EditorGUILayout.Foldout(aosFoldout, "Android", platformStyle);

            if (true == aosFoldout)
            {
                settings.storeCodeAOS   = EditorGUILayout.TextField(storeContent, settings.storeCodeAOS);
            ...
            if (true == standaloneFoldout)
            {
                ...
                settings.useWebview = EditorGUILayout.BeginToggleGroup(useWebviewContent, settings.useWebview);
                EditorGUILayout.EndToggleGroup();
            }
