...
namespace Toast.Gamebase.Mobile
{
    public class NativeGamebaseUtil : IGamebaseUtil
    {
        ...
        {
            public const string UTIL_API_SHOW_TOAST             = "gamebase://showToast";
        ...
        virtual public void ShowToast(string message, int duration)
        {
            NativeRequest.Util.AlertDialog alertDialog  = new NativeRequest.Util.AlertDialog();
            alertDialog.message                         = message;
            alertDialog.duration                        = duration;

            string jsonData = JsonMapper.ToJson(new UnityMessage(GamebaseUtil.UTIL_API_SHOW_TOAST, jsonData: JsonMapper.ToJson(alertDialog)));
        ...
        virtual public void ShowAlert(string title, string message)
        {
            ...
            string jsonData = JsonMapper.ToJson(new UnityMessage(GamebaseUtil.UTIL_API_SHOW_ALERT, jsonData: JsonMapper.ToJson(alertDialog)));
            messageSender.GetAsync(jsonData);
        }
        ...
        virtual public void ShowAlert(string title, string message, int handle)
        {
            ...
            string jsonData = JsonMapper.ToJson(new UnityMessage(GamebaseUtil.UTIL_API_SHOW_ALERT_EVENT, handle: handle, jsonData: JsonMapper.ToJson(alertDialog), gameObjectName: GamebaseUnitySDK.ObjectName, requestMethodName: "OnAsyncEvent"));
            messageSender.GetAsync(jsonData);
        }
