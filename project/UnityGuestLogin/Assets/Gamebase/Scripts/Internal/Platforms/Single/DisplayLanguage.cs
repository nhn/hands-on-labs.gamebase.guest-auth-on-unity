...
namespace Toast.Gamebase.Single
{
    public class DisplayLanguage
    {
        ...
        public IEnumerator DisplayLanguageInitialize()
        {
            ...
            {
                yield return GamebaseUnitySDKSettings.Instance.StartCoroutine(LoadLocalizedString());
            }
        }
        ...
        public string GetString(string key)
        {
            ...
            if (null == fieldInfo)
            {
                GamebaseLog.Error(string.Format("`{0}` {1}", key, GamebaseStrings.LOCALIZED_STRING_KEY_NOT_SUPPORTED), this, "GetString");
                return string.Empty;
            }
            ...
            if (true == string.IsNullOrEmpty((string)fieldInfo.GetValue(GetLocalizedStringVO())))
            {
                ...
                else
                {
                    ...
                    {
                        GamebaseLog.Warn(string.Format("`{0}` {1}", key, GamebaseStrings.LOCALIZED_STRING_KEY_NOT_FOUND), this, "GetString");
                        return string.Empty;
                    }
                    else
                    {
        ...
        private IEnumerator LoadLocalizedString()
        {
            ...
            {
                yield return GamebaseUnitySDKSettings.Instance.StartCoroutine(LoadWebFile(filePath));
            }
            else
            {
        ...
        private IEnumerator LoadWebFile(string filePath)
        {
            ...
            if (true == www.isDone)
            {
                if (200 == www.responseCode)
                {
                    ...
                    {
                        GamebaseLog.Warn(string.Format("error:{0}", www.error), this, "LoadLocalizedString");
                    }
                    else
                    {
                        if (true == string.IsNullOrEmpty(www.downloadHandler.text))
                        {
                            GamebaseLog.Warn(GamebaseStrings.LOCALIZED_STRING_EMPTY, this, "LoadLocalizedString");
                            yield break;
                        }

                        jsonString = www.downloadHandler.text;
                        localizedStrings = JsonMapper.ToObject<Dictionary<string, LocalizedStringVO>>(jsonString);
                        GamebaseLog.Debug(GamebaseStrings.LOCALIZED_STRING_LOAD_SUCCEEDED, this, "LoadLocalizedString");
                    }
                }
                else
                {
                    ...
                    {
                        GamebaseLog.Warn(string.Format("{0} responseCode:{1}.", GamebaseStrings.LOCALIZED_STRING_NOT_FOUND, www.responseCode), this, "LoadLocalizedString");
                    }
                    else
                    {
                        GamebaseLog.Warn(string.Format("{0} responseCode:{1}.", GamebaseStrings.LOCALIZED_STRING_LOAD_FAILED, www.responseCode), this, "LoadLocalizedString");
                    }
                }
            }
        }
        ...
        private void LoadLocalFile(string filePath)
        {
            ...
            {
                GamebaseLog.Warn(GamebaseStrings.LOCALIZED_STRING_NOT_FOUND, this, "LoadLocalizedString");
            }
        }
        #endregion
