...
namespace Toast.Gamebase
{
    public class AuthAdapterManager
    {
        ...
        public void IDPLogout(string providerName)
        {
            ...
            {
                GamebaseLog.Warn(GamebaseStrings.AUTH_ADAPTER_NOT_FOUND_NEED_SETUP, this, "IDPLogout");
                return;
            }
        ...
        public T GetIDPData<T>(string providerName, string methodName)
        {
            if (IsSupportedIDP(providerName) == false)
            {
                GamebaseErrorNotifier.FireNotSupportedAPI(this, methodName);
                return default(T);
            }

            if (HasAdapter(providerName) == false)
            {
                GamebaseLog.Warn(GamebaseStrings.AUTH_ADAPTER_NOT_FOUND_NEED_SETUP, this, "GetIDPData");
                return default(T);
            }
