...
//  TCGBGamebase
//
//  Created by NHNENT on 2016. 6. 24..
...
typedef void(^TransferKeyCompletion)(TCGBTransferKeyInfo* transferKey, TCGBError *error);
...
/**
 ...
 - (void)publishTransferKey {
     [TCGBGamebase issueTransferKeyWithExpiresIn:3600 completion:^(TCGBIssueTransferKey* transferKey, TCGBError *error) {
        NSLog(@"Published TransferKey => %@, error => %@", [transferKey description], [error description]);
     }];
 }
 
 @param expiresIn TransferKey is valid for expiresIn value in seconds.
 @param completion Handler to receive transferKey, regDate, expireDate and errors.
 @since Added 1.9.0.
 */
+ (void)issueTransferKeyWithExpiresIn:(long)expiresIn completion:(TransferKeyCompletion)completion;
...
/**
 ...
 - (void)transferOtherDevice {
     [TCGBGamebase requestTransferWithTransferKey:@"1Aie0198" completion:^(TCGBAuthToken *authToken, TCGBError *error) {
         NSLog(@"Transfered => %@",\nerror => %@", [authToken description], [error description]);
    }];
 }
 
 @param transferKey TransferKey received from old device.
 @param completion Handler that include guest login informations and errors.
 @since Added 1.9.0.
 */
+ (void)requestTransferWithTransferKey:(NSString *)transferKey completion:(LoginCompletion)completion;
